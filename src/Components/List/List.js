...
function List({ category }) {
  ...
  if(category === "food") {listItem = food;}
  else if(category === "activity") {listItem = activity;}
  else if (category === "attraction") {listItem = attraction;}
  else {listItem = lodging;}
  ...
  // css파일에도 모든 클래스이름에서 Food만 뺌
    const list = listItem.map((item) =>
        <div className='List-Item-div'><p className='List-Item'>
            🍚   [{item.name}]  👍🏻 <span className='List-like'>{item.like}</span>   {item.star}  {item.explanation}</p>
